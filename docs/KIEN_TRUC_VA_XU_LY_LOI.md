...
### 17. Cơ chế Đánh giá Tính Hợp lệ của Cache Model & Lỗi Lệch Phiên bản Client (`cache version mismatch`) -> Cơ chế Atomic Auto-Backup & Auto-Recovery cho File Token (`auths/`) — Chống lỗi Null Bytes (0x00) trên NTFS
* **Hiện tượng:**
  * Sau khi máy tính bị Sleep, mất điện ngỏ, crash hoặc tiến trình Proxy bị kết thúc đột ngột đúng vào thời điểm đang xóa/ghi đè token, một số file OAuth trong `auths/` (đặc biệt là Google Antigravity với chu kỳ refresh 1 giờ) biến thành toàn bộ byte rỗng `0x00` (Null bytes) hoặc file 0-byte.
  * Khi khởi động lại, CLIProxyAPI gặp lỗi `Invalid JSON` trên các file này và **tự động bỏ qua** chúng. Toàn bộ tài khoản biến mất khỏi Management Web UI mà không hề báo lỗi HTTP 401 nào — lỗi "âm thầm" nghiêm trọng nhất của hệ thống.
* **Bản chất kỹ thuật & Nguyên nhân gốc trên NTFS:**
...
* **Giải pháp chuẩn hóa toàn diện trong AIC (Bản phát hành v1.1.5):**
  1. *Cấu hình ghi đè Top-level `instructions` trong `~/.codex/config.toml` (`scripts/configure_codex_toml.py`):*
     * Khai báo trực tiếp `instructions = "You are Codex, an expert coding agent."` ở cấp cấu hình cao nhất của `config.toml`. Cấu hình này triệt tiêu hoàn toàn hằng số fallback nội tại của `codex.exe`, bảo đảm 100% request `/v1/responses` phát sinh từ Codex CLI đều mang chỉ dẫn sạch, tương thích hoàn toàn với gateway Antigravity.
  2. *Khử độc sâu lịch sử phiên chat đa cấu trúc (`scripts/sync_sessions.py`):*
     * Nâng cấp hàm `sanitize_session_item`: Nhận diện và làm sạch cả cấu trúc Dict `{"text": "..."}` lẫn chuỗi phẳng trong `base_instructions`, khử độc triệt để dòng metadata 0 và các thẻ `<model_switch>`.
     ...
     * Tích hợp kiểm tra toàn vẹn trong `verify_provider(..., check_instructions=True)` để phát hiện mọi dấu hiệu instruction chưa được làm sạch.
  3. *Khử độc template cache (`docs/models_cache_template.json`):*
     * Chuẩn hóa toàn bộ câu mở đầu thành `"You are Codex, an expert coding agent."`, bảo đảm `client_version` khớp với binary `codex.exe` hiện hành và khóa thuộc tính `Read-Only` cấp hệ điều hành.
  4. *Lệnh bảo trì tự động `aic repair` (`bin/aic.py`):*
     * Tự động phát hiện và giải phóng tiến trình `codex.exe` đang chạy ngầm (`kill_codex`) nhằm triệt tiêu hoàn toàn xung đột khóa tệp (file lock) mà không bắt người dùng phải thao tác đóng thủ công.
     * Tự động cập nhật `config.toml` bổ sung chỉ dẫn chuẩn `instructions`.
     * Đồng bộ nguyên tử file `models_cache.json` từ template và khóa Read-Only.
     * Khử độc toàn bộ lịch sử các phiên chat hiện có trong thư mục `sessions/`.
     * Tự động cân chỉnh lại biên cắt lineage (`realign_forked_lineages`) và xóa cache projection SQLite (`thread_history_1.sqlite`).
     * Kiểm tra toàn vẹn hệ thống trước khi kết thúc và trả về mã trạng thái chuẩn.
