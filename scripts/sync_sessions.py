...
def sanitize_instruction_text(text: str) -> tuple:
    """Sanitize known upstream-filtered phrases in system/developer instructions.
    Returns (new_text, was_modified)."""
    if not isinstance(text, str) or "based on GPT-5" not in text:
        return text, False

    new_text = text
    # 1. Full instruction templates generated by Codex CLI binary variants
    ...
    # 2. Generic based on GPT-5 within developer/instruction context
    new_text = new_text.replace("based on GPT-5", "an expert coding agent")

    return new_text, (new_text != text)


def sanitize_session_item(item: dict) -> tuple:
    """Sanitize harness-generated fields in a session item JSON object.
    Only modifies harness/system fields:
    - developer messages containing <model_switch> or instructions
    - top-level instruction fields in turn_context or session_meta
    Strictly preserves user messages, assistant responses, and tool call/output payloads.
    ...
    # 1. Top-level instructions in session_meta, turn_context, or root item
    instruction_keys = ("instructions", "base_instructions", "system_instructions", "developer_instructions", "instructions_template")
    for key in instruction_keys:
        if key in item:
            if isinstance(item[key], str):
                new_val, changed = sanitize_instruction_text(item[key])
                if changed:
                    item[key] = new_val
                    modified = True
            elif isinstance(item[key], dict) and "text" in item[key] and isinstance(item[key]["text"], str):
                new_val, changed = sanitize_instruction_text(item[key]["text"])
                if changed:
                    item[key]["text"] = new_val
                    modified = True
        if isinstance(payload, dict) and key in payload:
            if isinstance(payload[key], str):
                new_val, changed = sanitize_instruction_text(payload[key])
                if changed:
                    payload[key] = new_val
                    modified = True
            elif isinstance(payload[key], dict) and "text" in payload[key] and isinstance(payload[key]["text"], str):
                new_val, changed = sanitize_instruction_text(payload[key]["text"])
                if changed:
                    payload[key]["text"] = new_val
                    modified = True

    # 2. Developer messages (e.g. <model_switch>...</model_switch>)
    is_dev_msg = False
    target_msg = None

    if item_type == "message" and item.get("role") in ("developer", "system"):
        is_dev_msg = True
        target_msg = item
    ...
    if is_dev_msg and target_msg:
        content = target_msg.get("content")
        if isinstance(content, str):
            if "<model_switch>" in content or "based on GPT-5" in content:
                new_c, changed = sanitize_instruction_text(content)
                if changed:
                    target_msg["content"] = new_c
                    modified = True
        elif isinstance(content, list):
            for part in content:
                if isinstance(part, dict):
                    part_text = part.get("text")
                    if isinstance(part_text, str) and ("<model_switch>" in part_text or "based on GPT-5" in part_text):
                        new_pt, changed = sanitize_instruction_text(part_text)
                        if changed:
                            part["text"] = new_pt
                            modified = True
...
def has_unsanitized_fingerprint(item: dict) -> bool:
    """Returns True if item is a harness instruction or developer message that still contains 'based on GPT-5'.
    Ignores user, assistant, and tool messages."""
    if not isinstance(item, dict):
        return False
    ...
    for key in instruction_keys:
        if key in item:
            if isinstance(item[key], str) and "based on GPT-5" in item[key]:
                return True
            if isinstance(item[key], dict) and isinstance(item[key].get("text"), str) and "based on GPT-5" in item[key]["text"]:
                return True
        if isinstance(payload, dict) and key in payload:
            if isinstance(payload[key], str) and "based on GPT-5" in payload[key]:
                return True
            if isinstance(payload[key], dict) and isinstance(payload[key].get("text"), str) and "based on GPT-5" in payload[key]["text"]:
                return True
    ...
    if is_dev_msg and target_msg:
        content = target_msg.get("content")
        if isinstance(content, str) and ("<model_switch>" in content or "based on GPT-5" in content) and "based on GPT-5" in content:
            return True
        elif isinstance(content, list):
            for part in content:
                if isinstance(part, dict):
                    part_text = part.get("text")
                    if isinstance(part_text, str) and "based on GPT-5" in part_text:
                        return True
            ...
            for f in files:
                if f.endswith(".jsonl"):
                    ...
                    try:
                        ...
                        # Sanitize session_meta if it contains instruction fields
                        meta, meta_changed = sanitize_session_item(meta)
                        if meta_changed or needs_write:
                            lines[0] = json.dumps(meta, ensure_ascii=False) + "\n"
                            needs_write = True

                        # Process lines 1..N with structured parsing
...
def verify_provider(target_provider: str, codex_dir: Path, check_instructions: bool = False) -> int:
    target_provider = target_provider.strip().lower()
    if target_provider not in VALID_PROVIDERS:
        error(f"invalid provider '{target_provider}' (expected custom|openai)")
        return 2
                            ...
                            for line_idx, raw_l in enumerate(all_lines):
                                ...
                                try:
                                    ...
                                    if has_unsanitized_fingerprint(l_item):
                                        errors.append(f"Session {f} line {line_idx + 1} contains un-sanitized 'based on GPT-5' in developer instructions")
                                        break
                                except Exception:
                                    pass
                    except Exception as e:
...
def main() -> int:
    ...
    if not provider:
        error("usage: sync_sessions.py [--verify] [--check-instructions] <custom|openai> [--codex-dir <path>]")
        return 2

    codex_dir = get_codex_dir(args.codex_dir)

    if is_verify:
        return verify_provider(provider, codex_dir, check_instructions=args.check_instructions)
    else:
